<?php
    ...
    if($profile['batch'] == NULL){
        echo "<script>
        window.location.href= 'joinBatch';
        </script>";
    }
?>
<!DOCTYPE html>
  ...
  <div class="content-wrapper">
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Welcome <?php echo $profile['name']; ?></h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
    <?php
      $ongoingexamcheck = "SELECT * FROM submissions WHERE userid='$userid' AND (status=0 OR status=2 OR status=4)";
                  ...
                  <tbody>
                    ...
                    ?>
                      <tr>
                        ...
                        <?php
                          $examid = $ongoingexam['examid'];
                          $examquery = "SELECT * FROM exam WHERE examid='$examid'";
                          $examqueryexec = mysqli_query($conn, $examquery);
                          $examdetails = mysqli_fetch_array($examqueryexec);
                        ?>
                        <td><?php echo $examdetails['examduration']." mins"; ?></td>
                        <td><?php echo $ongoingexam['fullmarks']; ?></td>
                        <td>
                          <?php
                            if(($ongoingexam['status']==0) || ($ongoingexam['status']==2)){
                          ?>
                          <button type="button" class="btn btn-block btn-primary btn-xs" onclick="window.location.href='exam/attemptExam?examid=<?php echo $ongoingexam['examid']; ?>&intent=attempt'">Attempt</button>
                          <?php
                            }elseif($ongoingexam['status']==4){
                          ?>
                          <button type="button" class="btn btn-block btn-warning btn-xs" onclick="window.location.href='exam/uploadAnswerScript?examid=<?php echo $ongoingexam['examid']; ?>&submissionid=<?php echo $ongoingexam['submissionid']; ?>'">Upload Answer Script</button>
                          <?php
                            }
                          ?>
                          <!-- <button type="button" class="btn btn-block btn-danger btn-xs" onclick="window.location.href='exam/attemptExam?examid=<?php echo $ongoingexam['examid']; ?>&intent=notappear'" data-toggle="modal" data-target="#modal-default<?php echo $c; ?>">Mark as Not Appearing</button> -->
                          <button type="button" class="btn btn-block btn-danger btn-xs" data-toggle="modal" data-target="#modal-default<?php echo $c; ?>">Mark as Not Appearing</button>
                        </td>
                      </tr>
                    <?php
                      $c++;}
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">No ongoing exams today :)</h1>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
